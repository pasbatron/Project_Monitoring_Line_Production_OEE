[
    ...
    },
    {
        ...
        "z": "7d5d59cd7e772c15",
        "name": "Calculate Duration mc_fault",
        "func": "// Initialize context variables if not already set\nif (context.get('startTime') === undefined) {\n    context.set('startTime', null);\n}\nif (context.get('stopTime') === undefined) {\n    context.set('stopTime', null);\n}\nif (context.get('pauseTime') === undefined) {\n    context.set('pauseTime', null);\n}\nif (context.get('duration') === undefined) {\n    context.set('duration', 0);\n}\nif (context.get('pausedDuration') === undefined) {\n    context.set('pausedDuration', 0);\n}\n\n// Get the action from the input message\nlet action = msg.payload.action;\n\n// Handle the different actions\nif (action === \"start\") {\n    if (context.get('pauseTime')) {\n        // Resuming from pause\n        let pausedDuration = context.get('pausedDuration') + (new Date() - context.get('pauseTime'));\n        context.set('pausedDuration', pausedDuration);\n        context.set('pauseTime', null);\n        node.status({ fill: \"green\", shape: \"dot\", text: \"Resumed\" });\n    } else {\n        // Starting new\n        context.set('startTime', new Date());\n        context.set('stopTime', null);\n        context.set('duration', 0); // Reset duration on start\n        context.set('pausedDuration', 0); // Reset paused duration on start\n        node.status({ fill: \"green\", shape: \"dot\", text: \"Started\" });\n    }\n} else if (action === \"stop\") {\n    if (context.get('startTime')) {\n        context.set('stopTime', new Date());\n        let duration = (context.get('stopTime') - context.get('startTime') - context.get('pausedDuration')) / 1000; // Calculate duration in seconds\n        context.set('duration', duration);\n        msg.payload = { duration: duration };\n        node.status({ fill: \"red\", shape: \"dot\", text: `Stopped: ${duration} sec` });\n        return msg; // Send message with the duration\n    }\n} else if (action === \"pause\") {\n    if (context.get('startTime') && !context.get('pauseTime')) {\n        context.set('pauseTime', new Date());\n        node.status({ fill: \"yellow\", shape: \"dot\", text: \"Paused\" });\n    }\n} else if (action === \"reset\") {\n    context.set('startTime', null);\n    context.set('stopTime', null);\n    context.set('pauseTime', null);\n    context.set('duration', 0);\n    context.set('pausedDuration', 0);\n    node.status({ fill: \"blue\", shape: \"dot\", text: \"Reset\" });\n}\n\n// Do not send a message for start and reset actions\nreturn null;\n",
        "outputs": 1,
        "timeout": "",
        "noerr": 1,
        "initialize": "",
    ...
    },
    {
        ...
        "type": "function",
        "z": "7d5d59cd7e772c15",
        "name": "signal_loading_time",
        "func": "\nif (msg.payload[0] == \"30\") {\n    msg.payload = \"*signal35on_mcfault#\";\n    return msg;\n};\n\n\n\n\n",
        "outputs": 1,
        "timeout": 0,
        "noerr": 0,
        "initialize": "",
